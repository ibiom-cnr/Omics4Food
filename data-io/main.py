...
class FileUploader(object):
    ...
    def __init__(self):
        ...
        self.path=os.environ["UPLOAD_DIR"] # output_run_id in locale
        self.filenames=os.environ["OUTPUT_FILENAMES"].split(",") #lista filename delimitati da virgola -> tar gz output
        self.protocol=os.environ["OUTPUT_PROTOCOL"].lower() # swift+keystone
        self.url=os.environ["OUTPUT_ENDPOINT"] # https://cloud.recas.ba.infn.it:5000/v3 (forse va con v2)
        self.outpath=os.environ["OUTPUT_PATH"] # container hash del nome utente/ hash id progetto
        self.username=os.environ["OUTPUT_USERNAME"]
        self.password=os.environ["OUTPUT_PASSWORD"]
        
        #read optional env variables
        try:
          self.tenant=os.environ["OUTPUT_TENANT"] #omics4food
        except KeyError as e:
          self.tenant = ""
        try:
          self.region=os.environ["OUTPUT_REGION"] # recas-cloud
        except KeyError as e:
          self.region = ""
